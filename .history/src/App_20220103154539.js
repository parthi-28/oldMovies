...
export default function App() {
  ...
  function Starting({ movieName, posterURL, rating, summary }) {
    ...
    const summaryButton={display:"block"}
    const[like, setlike]=useState(1000)
    const [dislike, setDislike]=useState(0)
    const [toggle, setToggle]=useState({summaryButton})
    const toggleButton=()=>{
      if(toggle=={summaryButton}){
        setToggle()
      }
    }
    ...
    return (
      <div className="list">
         ...
         <h2>{movieName}</h2> 
          ...
          <p style={summaryButton}>{summary}</p>
          </div>
      </div>
    );
  }
  return (
    <div className="App">
      ...
      {students.map(({ title, imdb, picture, plot }) => (
        <Starting movieName={title} posterURL={picture} rating={imdb} summary={plot} key={imdb} ss/>
      ))}
    </div>
  );
}